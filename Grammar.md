...
The following table summarizes whether or not the `account` or `alias` or a unique match expression
is required:
...
* `NONE({n})` Any character not in an identifier -- still has to be valid for the identifier.
...
#### Several accounts with the same policy
